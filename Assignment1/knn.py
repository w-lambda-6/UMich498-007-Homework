...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
      for j in range(num_test):
        dists[i, j] = torch.sum((x_train_flattened[i]-x_test_flattened[j]).pow(2))
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(x_train):
        dist[i, :] = torch.sum((x_train_flattened[i]-x_test_flattened).pow(2), axis = 1)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_train_squared = torch.sum(x_train_flattened.pow(2), axis = 1).view(-1, 1)
    x_test_squared = torch.sum(x_test_flattened.pow(2), axis = 1).view(1, -1)
    m1 = (x_train_squared+x_test_squared)
    m2 = torch.mm(x_train_flattened, x_test_flattened)
    dist = m1 - 2.0*m2
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    for i in range(num_test): # something wrong here
        _, idxs = torch.topk(- dists[:, i], k)
        labels = y_train[idxs]
        vals, counts = torch.unique(labels, return_counts = True)
        max_id = (counts == torch.max(counts))
        y_pred[i] = torch.min(vals[max_id])
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return y_pred
...
def knn_cross_validate(
    ...
    for i in range(num_folds):
      idxs = [k for k in range(num_folds) if k != i]

      x_cat = []
      y_cat = []

      for n in idxs:
        x_cat.append(x_train_folds[n])
        y_cat.append(y_train_folds[n])

      x_train_fold = torch.cat(x_cat)
      y_train_fold = torch.cat(y_cat)

      classifier = KnnClassifier(x_train_fold, y_train_fold)
      k_to_accuracies[k].append(classifier.check_accuracy(x_train_folds[i], y_train_folds[i], k=k, quiet=True))
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
